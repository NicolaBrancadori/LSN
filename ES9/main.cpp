...
int main (){
	...
	ofstream pippo("citta.0");
	for(int i=0;i<cities.size();++i){
		pippo<<cities[i][0]<<" "<<cities[i][1]<<" "<<cities[i][2]<<endl;	
	}
	pippo.close();
	...
	for(int f=0;f<n_gen;++f){
		...
		//Qui calcolo e stampo L mediato sulla buona metà della popolazione
		double Lsum=0;
		for(int i=0;i<s.Pop.size()*0.5;++i){
			Lsum+=s.Pop[i].PathLength;
		}
		Lsum/=s.Pop.size()*0.5;
		L_ave_out<<Lsum<<endl;

		//Stampo L e il percorso corrispondente
		L_out<<s.Pop[0].PathLength<<" ";
		for(int i=0;i<s.Pop[0].Path.size();++i){
			L_out<<s.Pop[0].Path[i][2]<<" ";
		}
		L_out<<endl;
	}
	...
	BestPath.open("MigliorPercorso.dat");
	for (int i = 0; i < s.Pop[0].Path.size(); ++i){

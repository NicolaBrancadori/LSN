...
{
	double k_elastic = 5.;
	return -k_elastic*val*val/2.0 + pow(val,4);
}

double external_potential_prime(double val)
{
	double k_elastic =5.;
	return -k_elastic*val + 4*pow(val,3);
}

double external_potential_second(double val)
{
	double k_elastic=5.;
	return -k_elastic + 12*pow(val,2);
}
...
{	double Psi_a=exp(-pow(v-mu,2)/(2*s*s));
	double Psi_b=exp(-pow(v+mu,2)/(2*s*s));
	// return 0;
	return ( -1/(2*s*s) * (Psi_a*pow(v-mu,2)/(s*s) + Psi_b*pow(v+mu,2)/(s*s) - (Psi_a+Psi_b))  +  Pot()*(Psi_a+Psi_b) )  / (Psi_a+Psi_b);
}

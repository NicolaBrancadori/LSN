  ...
  for (int i=0; i<npart-1; ++i){
    ...
    for (int j=i+1; j<npart; ++j){
      ...
      for(int k=igofr;k<n_props;++k){
        if(dr>(k-igofr)*bin_size && dr<(k-igofr+1)*bin_size) walker[k]+=2;
      }
...
//g(r)
    ...
    for (int i=igofr;i<n_props;++i){
      double dv= 4/3*pi * ( pow((2*i+1-2*igofr)*bin_size,3) - pow((i-igofr)*bin_size,3) );
      gdir = blk_av[i]/blk_norm/rho/m_part/dv;
      glob_av[i] += gdir;
      glob_av2[i] += gdir*gdir;
      err_gdir = Error(glob_av[i],glob_av2[i],iblk);
      
      Gofr << gdir << endl;
      if (iblk==nblk) Gave << setw(wd)<< glob_av[i]/(double)iblk << setw(wd) << err_gdir << endl;
    }
